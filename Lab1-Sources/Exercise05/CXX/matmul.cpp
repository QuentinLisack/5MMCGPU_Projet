...
// ------------------------------------------------------------------
        ...
        for(int i = 0; i < COUNT; i++)
        {
            ...
            seq_mat_mul_sdot(N, h_A, h_B, C);



            run_time  = (static_cast<double>(timer.getTimeMilliseconds()) / 1000.0) - start_time;
            results(N, C, run_time);
        }
...
// ------------------------------------------------------------------
       ...
       main     // Execute the kernel over the entire range of C matrix elements ... computing
            ...
            for( int i = 0; i < N; i++) 
                for(int j = 0; j<N; j++)
                    if(std::abs(C[i + j * N] - h_C[i * N + j])< 0.000001)
                        error++;
